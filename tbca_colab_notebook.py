...
# **Cell 1 — Εγκατάσταση πακέτων** ##
Εγκατάσταση όλων των απαραίτητων βιβλιοθηκών. Χρησιμοποιούμε τις τελευταίες εκδόσεις των transformers και huggingface_hub για συμβατότητα με το Colab. Επαλήθευση διαθεσιμότητας GPU (Tesla T4).
"""

# Cell 1 — Εγκατάσταση (έκδοση συμβατή με Colab 2025)
...
Ανέβασμα του αρχείου kaggle.json για πρόσβαση στο Kaggle API. Το αρχείο τοποθετείται στον κατάλληλο φάκελο και λαμβάνει τα σωστά permissions.
"""
...
"""## **Cell 3 — Λήψη και καθαρισμός dataset**
Λήψη του dataset "Amazon Laptop Review" από το Kaggle (511 reviews) - URL: https://www.kaggle.com/datasets/ashwinishet/amazon-laptop-review

Τυποποίηση ονομάτων στηλών (comments → text), αφαίρεση κενών τιμών και πολύ σύντομων κειμένων.
Τελικό καθαρό dataset: 413 reviews.
"""
...
"""## **Cell 3b — Στρωματοποιημένο δείγμα**
Επιλογή 200 reviews με διατήρηση της κατανομής των star ratings (στρωματοποιημένη δειγματοληψία).
Εξασφαλίζει αντιπροσωπευτικότητα του δείγματος ως προς όλες τις κατηγορίες ικανοποίησης.
"""
...
"""## **Cell 4 — Φόρτωμα ABSA μοντέλου**
Φόρτωμα του DeBERTa-v3-base fine-tuned στο SemEval-2016 Task 5 Laptop14 benchmark (Pontiki et al., 2016).
Θεωρητική σύνδεση (Section 2.3 paper): το μοντέλο αντιστοιχεί στο BERT-based ABSA pipeline που αναφέρεται στη βιβλιογραφία.
"""
...
Για κάθε review και για κάθε attribute, το μοντέλο δέχεται input της μορφής aspect [SEP] review text και επιστρέφει sentiment score sij∈[−1,+1]s_{ij} \in [-1, +1] sij​∈[−1,+1].

Τα 5 attributes (battery life, display quality, performance, build quality, value for money) αντιστοιχούν στις διαστάσεις xix_i xi​ του MAUT (Eq. 10).

Αποτέλεσμα: 1000 aspect-sentiment παρατηρήσεις.
"""
...
"""## **Cell 4c — Φιλτράρισμα αναξιόπιστων scores**

Διατήρηση μόνο scores με ∣sij∣>0.6|s_{ij}| > 0.6 ∣sij​∣>0.6 για εξάλειψη αβέβαιων εκτιμήσεων.

Αποτέλεσμα: 886 αξιόπιστες παρατηρήσεις από 186/200 reviews (93%).
"""

# Cell 4c — Φιλτράρισμα αναξιόπιστων scores
# Κρατάμε μόνο scores με υψηλό confidence (|score| > 0.6)
# και αποφεύγουμε το πρόβλημα "όλα θετικά/αρνητικά"

df_absa_filtered = df_absa[df_absa['score'].abs() > 0.6].copy()

# Έλεγχος: πόσα reviews έχουν τουλάχιστον 1 mapped aspect
...
"""## **Cell 5 — Κατασκευή feature matrix**

Μετατροπή των aspect-sentiment scores σε feature matrix X∈R186×5\mathbf{X} \in \mathbb{R}^{186 \times 5} X∈R186×5 μέσω pivot (μέσος όρος scores ανά review × attribute).

Συγχώνευση με τα normalised star ratings y∈[0,1]y \in [0,1] y∈[0,1].

Αυτή η matrix αντιστοιχεί στο X~\tilde{\mathbf{X}} X~ της Eq. 15 (neural linear layer).
"""
...
"""## **Cell 6 — Prospect Theory debiasing (Section 7.2)**

Εφαρμογή της inverse PT correction με λ=2.25\lambda = 2.25 λ=2.25 και α=0.88\alpha = 0.88 α=0.88 (Tversky & Kahneman, 1992).

Τα αρνητικά scores συμπιέζονται (π.χ. −0.796→−0.364-0.796 \rightarrow -0.364
−0.796→−0.364) για διόρθωση της υπερ-εκπροσώπησης αρνητικών reviews στο UGC λόγω loss aversion.
"""
...
"""## **Cell 7 — MAUT utility estimation (Section 6.3)**

Εκτίμηση των part-worth weights **w^i\hat{w}_i w^i**​ μέσω Ridge regression (αντίστοιχο του linear layer, Eq. 15).

**Normalisation** ώστε **∑iw^i=1\sum_i \hat{w}_i = 1** **∑i​w^i​=1 (MAUT constraint)**.

**Cross-validated R2=0.563R^2 = 0.563 R2=0.563** επιβεβαιώνει εξηγητική δύναμη.

Το debiasing μετατοπίζει τα weights στη θεωρητικά προβλεπόμενη κατεύθυνση (performance: +0.237+0.237 +0.237, build quality: −0.166-0.166 −0.166).
"""
...
"""## **Cell 8 — Softmax / MNL choice probabilities (Section 6.1)**

Υπολογισμός συστηματικών utilities **V^j=w^⊤xj\hat{V}_j = \hat{\mathbf{w}}^\top \mathbf{x}_j V^j​=w^⊤xj**​ και μετατροπή σε MNL choice probabilities μέσω softmax (Proposition 6.1).

Το Laptop B (Performance) επιλέγεται με P=35.2%P = 35.2\% P=35.2%, συνεπές με το υψηλό debiased weight του performance (w^=0.284\hat{w} = 0.284 w^=0.284).
"""
...
"""## **Cell 9 — Αξιωματική συνέπεια (Propositions 6.1–6.2)**

Επαλήθευση 4 αξιωματικών απαιτήσεων σε πραγματικά δεδομένα:
...
"""# **Cell 10 — Γραφήματα για το paper**

Παραγωγή 3-panel figure (300 dpi):

(α) σύγκριση raw vs debiased part-worth weights,

(β) PT debiasing effect Δ\Delta
Δ ανά attribute,

(γ) softmax choice probabilities.

Αποθήκευση σε PDF και PNG.
"""
...
"""# **Cell 11 — Export αποτελεσμάτων**

Εξαγωγή LaTeX tables έτοιμων για copy-paste στο paper (tbca_tables.tex), structured results (tbca_results.json), και figures (tbca_results.pdf/png).
"""
